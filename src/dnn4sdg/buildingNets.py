...
from folders import train_data_provider, test_data_provider
...
def main():
    ...
    if args.debug:
        ...
        if not args.oldformat:
            features_train,labels_train=train_data_provider()
            features_test,labels_test=test_data_provider()                            
        else:
            dataset = DATASETS['debug']            
            features, _, labels, _ = preprocess_data(
                args.patch_size, dataset=dataset)
        ...
        try:
            if args.oldformat:
                ...
                features_train, features_test, labels_train, labels_test = preprocess_data(
                    args.patch_size, dataset=dataset, args.distribution, only_cache=load_from_cache)
    ...
    if args.train_model:
        model = train_model(
            ...
            model_id, nb_epoch = args.epochs,
            checkpoints = args.checkpoints,
            tensorboard = args.tensorboard, earlystop = args.earlystop)
    
    if args.evaluate_model:
       
        evaluate_model(model, features_test, labels_test, args.patch_size,
