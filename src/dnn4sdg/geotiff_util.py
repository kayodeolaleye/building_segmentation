...
def data_provider(feature_data, label_data, patch_size, path_to_geotiff, path_to_geotiff_bitmap, distribution):
    ...
    for (row, col) in patch_indexes:
        ...
        if in_bounds:
            ...
            if new_patch_label.sum() * 100/np.product(patch_size, patch_size) >= distribution:
