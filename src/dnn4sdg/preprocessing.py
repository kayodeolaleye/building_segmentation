...
from geotiff_util import reproject_dataset, data_provider
from io_util import get_file_name, save_patches, save_bitmap, load_bitmap
import numpy as np



def preprocess_data(patch_size, dataset, distribution, only_cache=False):
    ...
    features_test, labels_test = extract_features_and_labels(
        dataset["test"], patch_size, distribution, only_cache)
    
    return features_train, features_test, labels_train, labels_test


def extract_features_and_labels(dataset, patch_size, distribution, only_cache=False):
    ...
    for geotiff_path, geotiff_bitmap_path in dataset:
        ...
        patched_features, patched_labels = create_patched_features_and_labels(
            geotiff_path, geotiff_bitmap_path, patch_size, distribution, only_cache)
...
def create_patched_features_and_labels(geotiff_path, geotiff_bitmap_path, patch_size, dist,
                                           only_cache=False,cache_path='/tmp'):
    ...
    patched_bands, patched_bitmap = data_provider(bands, building_bitmap, patch_size, wgs84_path, geotiff_bitmap_path, dist)
...
def create_bitmap(satellite_path, satellite_bitmap_path,  raster_dataset,cache_path='/tmp'):
