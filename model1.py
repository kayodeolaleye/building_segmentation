...
import os
import numpy as np
from folders import MODELS_DIR, TENSORBOARD_DIR
from io_util import save_makedirs, save_model
print("keras: ", keras.__version__)
print("tensorflow: " ,tf.__version__)
np.random.seed(123)

def train_model(model, features, labels, patch_size, model_id,
...
def init_model(patch_size, model_id, architecture='one_layer',
               ...
               stride_2=(1,1), learning_rate=0.01,
               momentum=0.9, decay=0.002):
    ...
    """if architecture == 'one_layer':
        ...
        #model.add(Dense(4096, input_shape=(patch_size, patch_size, num_channels), kernel_initializer='glorot_uniform'))
    ...
    if architecture == 'one_layer':
        ...
        model.add(Conv2D(12, (3, 3), subsample=(1,1), input_shape=(patch_size, patch_size, num_channels), padding='same'))
        model.add(Activation('relu'))        
        model.add(Conv2D(12, (3,3), subsample=(1,1), padding='same'))
        model.add(Activation('relu'))
        model.add(Conv2D(12, (3,3), subsample=(1,1), padding='same'))
        model.add(Activation('relu'))
        model.add(Conv2D(1, (1,1), subsample=(1,1), padding='same'))
        model.add(Activation('relu'))
        model.add(Flatten())
